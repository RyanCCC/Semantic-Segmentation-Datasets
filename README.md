...
5. 生成完成后的文件架构如下：JPEGImages保存的是原图。SegmentationClass保存的是npy的文件，是原图的标签，可以用于训练，如何训练.npy文件后面再研究。SegmentationClassPNG保存的是标签，是单通道的图像，以调色板格式保存的。SegmentationClassVisualization保存的是原图+标签，用于可是化打标签的结果。class_names.txt用于保存标签的类别。
